...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    function isUprightFace(landmarks) {
        ...
        // Rule 1: Nose must be below eyes
        if (noseTip.y < eyeMidY) return false;

        // Rule 2: Nose must not be too low (user looking down or bent)
        if (noseTip.y > jawBottom.y - 20) return false;

        // Rule 3: Eyes must be horizontally aligned (no head tilt)
        const eyeAlignmentDiff = Math.abs(leftEyeCenter.y - rightEyeCenter.y);
        if (eyeAlignmentDiff > 10) return false;

        // Rule 4: Nose must be horizontally centered (face facing camera)
        const noseCenterOffset = Math.abs(noseTip.x - eyeMidX);
        if (noseCenterOffset > 20) return false;
    ...
    async function detectBlink() {
        ...
        if (detection) {
            ...
            if (!isUprightFace(landmarks)) {
                statusText.textContent = "Please keep your face upright and look straight";
                videoContainer.style.borderColor = "orange";
                requestAnimationFrame(detectBlink);
                return;
            }
