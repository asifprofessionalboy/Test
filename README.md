...
public class FaceRecognitionService
{
    ...
    private readonly string _faceNetPath;

    public FaceRecognitionService()
    {
        _protoPath = Path.Combine("wwwroot", "Models", "deploy.prototxt");
        _modelPath = Path.Combine("wwwroot", "Models", "res10_300x300_ssd_iter_140000.caffemodel");
        _faceNetPath = Path.Combine("wwwroot", "Models", "facenet.onnx");
    }
    ...
    private float[] ExtractFaceEmbedding(Bitmap image)
    {
        ...
        var net = DnnInvoke.ReadNetFromCaffe(_protoPath, _modelPath);
        ...
        for (int i = 0; i < data.GetLength(2); i++)
        {
            ...
            if (confidence > 0.6f)
            {
                ...
                return RunFaceNetEmbedding(face);
            }
        }
    ...
    private float[] RunFaceNetEmbedding(Mat faceMat)
    {
        Bitmap bmp = MatToBitmap(faceMat);
        ...
        using var session = new InferenceSession(_faceNetPath);
